...
# Determine the 'Fraudulent' value using the following rules:
# - If the job title is missing or extremely vague, set "Fraudulent" to "yes".
# - If the salary is abnormally high or unrealistic for the job title, set "Fraudulent" to "yes".
# - If the job post includes overly generic descriptions, no company logo, missing qualifications, or suspicious phrases like "no experience needed but earn lakhs", "instant joining with no interview", or "click this link to apply", set "Fraudulent" to "yes".
# - If 50% or more of the extracted values (excluding the 'Fraudulent' field itself) are empty strings ("") or zeros (0), set "Fraudulent" to "yes".
# - Otherwise, if the job post appears reasonable and consistent, set "Fraudulent" to "no".
# """
