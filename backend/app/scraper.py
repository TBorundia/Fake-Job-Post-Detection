...
#             print(json.dumps(encoded_job_data, indent=4))
...
class JobScraper:
    ...
    def _scrape_naukri(self, url):
        ...
        try:
            with sync_playwright() as pw:
                # Get available browsers and launch the first available one
                browsers = [pw.chromium, pw.firefox, pw.webkit]
                browser = next((b.launch(headless=True) for b in browsers if b), None)
                
                if not browser:
                    raise Exception("No supported browser found in Playwright.")
